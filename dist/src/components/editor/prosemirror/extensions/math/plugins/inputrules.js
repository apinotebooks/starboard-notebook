...
export function inlineInputRule(pattern, nodeType, getAttrs) {
...
export function blockInputRule(pattern, nodeType, getAttrs) {
    return new InputRule(pattern, (state, match, start, end) => {
        const $start = state.doc.resolve(start);
        const attrs = getAttrs instanceof Function ? getAttrs(match) : getAttrs;
...
export function buildMathInputRules(schema) {
    return inputRules({
        rules: [
